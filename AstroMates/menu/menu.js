/* =========================================================
   ESTRUCTURA DE DATOS PARA EL MENÚ NAVEGABLE DE ASTROMATES
   ========================================================= */
...
/* =========================================================
   FUNCIÓN REUTILIZABLE PARA RESOLVER RUTAS DE ENLACE
   ========================================================= */
function resolverRutaEnlace(url) {
  if (!url) return '#';
  if (url.startsWith('http://') || url.startsWith('https://')) return url;

  // Si abres mediante un servidor local (ej. Live Server de VS Code)
  if (window.location.protocol.startsWith('http')) {
    return url.startsWith('/') ? url : '/' + url;
  }

  // Si estás abriendo archivos directamente en el navegador (protocolo file://)
  const pathCompleto = decodeURIComponent(window.location.pathname);
  const carpetaRaiz = '/AstroMates';
  const posRaiz = pathCompleto.indexOf(carpetaRaiz);

  if (posRaiz !== -1) {
    const rutaBaseRaiz = pathCompleto.substring(0, posRaiz + carpetaRaiz.length);
    const rutaLimpia = url.startsWith('/') ? url : '/' + url;
    return 'file://' + rutaBaseRaiz + rutaLimpia;
  }
      ...
      <ul class="submenu">
        ${mod.temas.map(tema => `
          <li class="submenu-header">${tema.nombre}</li>${tema.subtemas.map(sub => `
            <li>
              ${sub.link 
                ? `<a href="${resolverRutaEnlace(sub.link)}">${sub.titulo}</a>` 
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 1. Inyecta la barra de navegación
  if (headerContainer) {
    headerContainer.innerHTML = `
      <nav class="navbar">
        <ul class="nav-list">
          <li class="nav-item"><a href="${resolverRutaEnlace('/index.html')}" class="active-btn">Inicio</a></li>
  ...
  // 3. Renderiza MathJax si hay notación matemática
